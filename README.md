...
- [x] 本地划线、摘录和笔记：WebView 精确选区持久化，支持划线/摘录/笔记、原文定位与删除，并保存标题、作者、来源链接和时间。
- [x] 知识卡片：标注一键生成卡片，保留原文快照与来源，支持我的理解、标签、复习状态、全局管理和回到原文。
- [x] 回答/文章基础导出能力。
...
- [x] 本地划线、摘录和笔记：保存出处、标题、作者、链接和时间。
- [x] 知识卡片：原文摘录、我的理解、标签、复习状态，并与笔记联动。
- [x] 本地全文搜索：覆盖已读、收藏、离线缓存、稍后阅读、笔记和评论草稿；支持知识卡片与离线评论命中、来源胶囊和结果回跳。
- [x] 稍后阅读增强：可选离线正文保存、批量管理、排序与已读状态。
- [x] 评论区增强：作者评论高亮、作者回复聚合、高赞/争议/认真讨论筛选；争议仅按回复/点赞结构信号判断，不做情绪或立场推断。
- [x] 评论回复表情与回复辅助：82 个知乎表情支持光标/选区插入，提供离线可用的常用回复辅助短语，并继续复用实时草稿与离线发送队列。

### P2 — AI 与推荐

- [x] 文章/回答 AI 分析面板：设置中可配置 OpenAI 兼容 Base URL / API Key / 模型并自动获取模型列表；详情页 AI 小球按需分析总结、关键词、核心观点、信息密度、收藏价值、AI 写作特征风险、综合评价和需核实点。
- [x] AI 摘要第一版：详情页“本地摘要”完全在本机从正文提取小标题、稳定关键词和代表性核心句，不调用外部 API。
- [x] AI 摘要第二版：外部可选模型一次请求同时生成一句话总结、严格 3 条核心观点、反方/反例视角和适合谁看，并复用现有 AI 分析面板。
- [x] 内容质量评分：AI 同一次正文分析给出结构清晰度、标题党风险、广告/软文风险和模板化程度；评论区质量基于本地/首屏真实评论样本独立评分，评论正文不发送给外部模型。
